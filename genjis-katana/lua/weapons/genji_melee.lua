...
SWEP.ViewModelBoneMods = {
    ["ValveBiped.base"] = {
        ...
        pos = Vector(0, -0.86, -0.005),
        angle = Angle(0, 0, 0),
    },
    ["ValveBiped.Bip01_L_UpperArm"] = {
        scale = Vector(1, 1, 1),
        pos = Vector(-1.859, -1.543, -2.842),
        angle = Angle(0, 0, 0),
    },
    ["ValveBiped.Bip01_L_Hand"] = {
        scale = Vector(1, 1, 1),
        pos = Vector(-0.389, 0, 0),
        angle = Angle(-8.622, -51.949, 0),
    },
}
SWEP.Slot = 7
SWEP.UseHands = true
...
SWEP.WElements = {
    ["sword"] = {
        ...
        rel = "",
        pos = Vector(3.719, 1.939, -1.162),
        angle = Angle(180, 180, 0),
        size = Vector(1.016, 1.016, 1.016),
...
if CLIENT then
    ...
    function SWEP:ViewModelDrawn()
        ...
        for k, name in ipairs(self.vRenderOrder) do
            ...
            if v.type == "Model" and IsValid(model) then
                model:SetPos(
                    pos + ang:Forward() * v.pos.x + ang:Right() * v.pos.y + ang:Up() * v.pos.z
                )
    ...
    function SWEP:DrawWorldModel()
        ...
        for k, name in pairs(self.wRenderOrder) do
            ...
            if v.bone then
                ...
                pos, ang =
                    self:GetBoneOrientation(self.WElements, v, bone_ent, "ValveBiped.Bip01_R_Hand")
            end
            if not pos then
                continue
            end
    ...
    function SWEP:GetBoneOrientation(basetab, tab, ent, bone_override)
        ...
        if tab.rel and tab.rel ~= "" then
            ...
            if
                IsValid(self.Owner)
                and self.Owner:IsPlayer()
                and ent == self.Owner:GetViewModel()
                and self.ViewModelFlip
            then
    ...
    function SWEP:CreateModels(tab)
        ...
        for k, v in pairs(tab) do
            if
                ...
                and string.find(v.model, ".mdl")
                and file.Exists(v.model, "GAME")
            then
